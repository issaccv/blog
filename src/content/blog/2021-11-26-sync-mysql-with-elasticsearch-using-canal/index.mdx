...
在使用docker部署canal前我个人建议先在容器外跑一遍，因为canal的Dockerfile写的实在是不怎么样。正如上文提到的，canal使用了“生产-消费”模型，在本文的场景中，生产者是MySQL，而消费者是Elasticsearch。而canal和canal-adapter则在生产者和消费者之间作为消息传递的通道。```MySQL-canal-canal-adapter-Elasticsearch```canal通过对消息队列的支持实现生产消费的进一步解耦，支持Kafka和RocketMQ。同时也支持Promethus的指标监控，方便实现异常报警。```MySQL-canal-Kafka/RocketMQ-canal-adapter-Elasticsearch```canal也原生支持[keepalived](https://www.redhat.com/sysadmin/keepalived-basics)高可用，在局域网环境下通过VRRP做到无缝切换主备节点。

## How to do

```markdown
Environment:-Canal:v1.1.5-Canal-adapter:v1.1.5-Elasticsearch:v7.14.2-MySQL:8.0.x
```
